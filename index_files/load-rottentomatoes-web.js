...
mps.inViewport = function(s, map) {
  ...
  if(!mps._bool(mpsopts.activeload) || (mps._bool(mpsopts.activeload) === true && mps._activerefresh._focus === true)) {
    var win = window, winDocElem = win.document.documentElement, docHeight = mps._getDocHeight(), vp = mps._vp(),
        ...
        atfPad = mps._detect && typeof mps._detect.getATFPadding(slotname) == 'number' ? mps._detect.getATFPadding(slotname) + defaultPad : 0,
        atfEnabled = mps._bool(mpsopts.detection_above_the_fold) && mps._detect && (scY < 250),
...
/* [2018-09-19] <START> Header Bidding (v7) */ mps.hb=mps.hb||{}; mps.hb.executed=0; mps.hb._version=7;
/*====[config]============================*/
mps.hb.a9pubid = '3219';
mps.hb.ixjs = '';
mps.hb.moatyi = 'nbcuyieldheader7581548001';
/*========================================*/
// @TODO A9 for loadset > 0
// @TODO !c=headerbid
mps.hb.a9timeout = 2000;
mps.hb.kvinsertcheckinterval = 250;
mps.hb.initialrefreshdelay = 50;
...
mps.hb.execute = function() {
  ...
  mps._reqs.hb=mps._reqs.hb||{}; mps.hb.executed++; mps.hb.loadedads=0; mps.hb._gptrefreshes=mps.hb._gptrefreshes||{}; mps.hb._gptrefreshes.initial=mps.hb._gptrefreshes.initial||[]; mps.hb._gptrefreshes.adexecute=mps.hb._gptrefreshes.adexecute||[];
  mps._d('[mps/hb] Starting Execution');
  if(mps.hb.moatyi) {
    mps._d('[mps/hb] MoatYI: Load ('+mps.hb.moatyi+')');
    ...
    window['moatYieldReady'] = function() {
      mps._d('[mps/hb] MoatYI: moatYieldReady');
      if(debugmode.log && window.moatPrebidApi && typeof(window.moatPrebidApi.enableLogging)=='function') window.moatPrebidApi.enableLogging();
      mps._queue.gptloaded.push(function() {
        mps.hb.moatyislots=mps.hb.moatyislots||[];
        for(var i in mps._advars[mps._loadset]) {
          var adobj = gpt[mps._advars[mps._loadset][i]];
          if(i=='_oop' || !adobj || !adobj.setTargeting) continue;
          mps.hb.moatyislots.push(i);
        }
        mps._d('[mps/hb] MoatYI: slotDataAvailable='+window.moatPrebidApi.slotDataAvailable());
        ...
        mps._d('[mps/hb] MoatYI: Loaded (moatYieldReady)');
        mps._reqs.hb['moatyi_loaded']=mps._elapsed('',true);
        mps.hb.startAdExecution();
      })
    }
    mps._loadJS(mps.hb.moatyiurl);
  }
  if(mps.hb.ixjs) {
  ...
  mps._queue=mps._queue||{}; mps._queue.gptloaded=mps._queue.gptloaded||{}; mps._queue.mpsready=mps._queue.mpsready||{};
  mps._queue.mpsready.push(function() {
  ...
  mps._queue.gptloaded.push(function() {
    ...
    if(mps.hb.a9pubid) {
      ...
      for(var i in mps._advars[loadset]) {
        if(i=='_oop') continue;
        var adobj = gpt[mps._advars[loadset][i]], slot = {};
      ...
      if(mps.hb.a9slots.length) {
        ...
        apstag.fetchBids({slots:mps.hb.a9slots, timeout:mps.hb.a9timeout}, function(bids){
          ...
          mps._d('[mps/hb] A9: Loaded (fetch bids)');
...
mps.hb.startAdExecution = function() {
  ...
  if(mps.hb._adexecstarted) {
    mps._d('[mps/hb] SKIP startAdExecution (already called)');
    return false;
  }
  ...
  // nbcnews-web fix for <= 3.57.0
  mps.adController && mps.adController.insertAds();
  var loadset = mps._loadset;
  mps._queue.gptloaded.push(function(){
    var refreshobs=[]; mps.hb._initialads=mps.hb._initialads||{}; mps.hb._initialads[loadset]=[];
...
mps.execute_lazyload_js = function() {
  ...
  mps._detect.scroll=mps._detect.scroll||{};
  mps._detect.scroll.updateinterval=mps._detect.scroll.updateinterval||500;
  ...
  mps._detect.load = mps.debounce(function() {
    ...
    if(pending.length) {
      for(var a=0; a<pending.length; a++) {
        ...
        // TODO: too much noise if debug message in mps._detect.load
        ...
        if(typeof gptdiv === 'string' && typeof document.getElementById(gptdiv) == 'object' && ((_ads.loadset !== 0 || typeof(mps._detect.detectionelement[_ads.slot]) !== 'boolean') || mps._detect.detectionelement[_ads.slot] === true) && mps.inViewport(document.getElementById(gptdiv))) {
          ...
          mps._detect.tracking[_ads.loadset][_ads.slot] = mps._detect.distanceToViewport(mps._select('#' + gptdiv));
  ...
  /**
   * Calculates Scroll Velocity Average (in pixels)
   * Uses datapoints: {@link mps._detect.scroll.velocitypoints}
   * Saved interval in ms {@link mps._detect.scroll.updateinterval}
   * @return velocity {number}
   */
  mps._detect.scroll.calculate = function(){
    mps._detect.scroll.velocitypoints = mps._detect.scroll.velocitypoints || {};
    mps._detect.scroll.velocitypointsreal = mps._detect.scroll.velocitypointsreal || {};
    mps._detect.scroll.velocity = mps._detect.scroll.velocity || 0;
  
    var points = mps._detect.scroll.velocitypoints,
        realpoints = mps._detect.scroll.velocitypointsreal,
        finaltotal = 0,
        newpoint = document.documentElement.scrollTop,
        now = parseInt(mps._elapsed('',true)),
        last = parseInt(mps._keys(points)[mps._keys(points).length-1]) || 0,
        currenttotal = mps._detect.scroll.totalpoints(realpoints);
  
    if(mps._detect.scrolling && now >= (last + (mps._detect.scroll.updateinterval-100))){
      points[now] = Math.round(Math.abs(currenttotal - newpoint));
      // keep negatives for calculating currenttotal
      realpoints[now] = Math.round((currenttotal - newpoint))*-1;
      finaltotal = mps._detect.scroll.totalpoints(realpoints);
      mps._detect.scroll.velocity = Math.round(finaltotal / mps._keys(points).length);
      mps._detect.scroll.velocitypoints = points;
      mps._detect.scroll.velocitypointsreal = realpoints;
    }
    return mps._detect.scroll.velocity;
  };
  
  /**
   * Calculates Total Velocity from all data points, if empty return 0
   * @param points
   * @return {number}
   */
  mps._detect.scroll.totalpoints = function(points){
    if(mps._keys(points).length>0) {
      return mps._keys(points).reduce(function (a, b) {
        return a + points[b];
      },0);
    }else {
      return 0;
  ...
  mps._detect.getATFPadding = function(slot){
    if(typeof slot === 'string' && mps._ext.loaded && mps.response.dart.params.adunits[slot]){
      var sizes = mps.response.dart.params.adunits[slot].sizes;
      var fluidIndex = sizes.indexOf('fluid');
      if(fluidIndex > -1){
        sizes.splice(fluidIndex, 1);
        mps._d('^[mps:Detect] getATFPadding => REMOVE fluid size: '+slot);
      }
    } else {
      mps._d('^[mps:Detect] getATFPadding => invalid param or slot disabled: '+slot);
      return false;
    }
  ...
  mps._detect.forceLoad = function(slot, loadset) {
    ...
    if (refresh && mps._detect._refresh[loadset][slot].length) {
      ...
      if (mps.refreshAds(slot, loadset)) {
        mps._detect.tracking[loadset][slot] = mps._detect.distanceToViewport(mps._select('#' + mps._detect._refresh[loadset][slot])) ? mps._detect.distanceToViewport(mps._select('#' + mps._detect._refresh[loadset][slot])): {};
  ...
  if(mps._detect.velocityinterval) {
    clearInterval(mps._detect.velocityinterval);
  }
  
  mps._detect.interval = setInterval(mps._detect.load, mps._detect.intVal);
  
  /**
   * Sets an interval equal to ..._detect.scroll.updateinterval (default: 500)
   * @type {*}
   */
  mps._detect.velocityinterval = setInterval(function() {
    if(mps._detect.scrolling) {
      mps._detect.scrolling = false;
      mps._detect.scroll.calculate();
    }
  }, mps._detect.scroll.updateinterval);
  ...
  if(window.addEventListener) {
    ...
    mps._scroll = window.addEventListener('scroll', mps._detect.scroll.calculate, {passive:true});
